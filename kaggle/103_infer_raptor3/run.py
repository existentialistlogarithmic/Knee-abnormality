...
def main():
    ...
    #
    # WHY IT EARNS A VOTE when E117's six same-author checkpoints did not. On the
    # 58 it scores 0.9093 against our CoAtNet composite's 0.9223 -- behind -- but
    # it correlates 0.872 with us while our own four arms sit at 0.905-0.986 with
    # each other. The blend buys DISAGREEMENT, not rank, and this is the first
    # public asset found outside that band while close in quality. Rank-meaned
    # 50/50 with the CoAtNet composite offline it gave 0.9287, +0.0064, CI
    # [-0.0064, +0.0187], P(better) 0.841. Nothing was fitted on the 58.
    #
    # IT CANNOT REUSE OUR VOLUMES. Its geometry contract declares six slot
    # budgets (20, 12, 12, 8, 16, 8) and a VARIABLE 48-76 windows per study,
    # against our fixed five slots and k_eval 62. It reads DICOM itself.
    #
    # AND IT CANNOT BE A SEPARATE KERNEL: E085's guard means a mounted kernel
    # supplies its last SAVED output, frozen at the 3-study visible run, so a
    # CSV-chained blend would submit three rows. It has to run here.
    #
    # We DRIVE their entry point rather than reimplement it. Their
    # `run_submission` self-discovers the test set and writes a submission-shaped
    # CSV; reimplementing six-slot windowing from their contract would be a
    # second chance to get it subtly wrong for no benefit.
    import importlib
    import sys as _sys

    # `find_all_markers`, not a one-level glob. /kaggle/input is NESTED --
    # competitions/<slug>, datasets/<owner>/<name>, notebooks/<owner>/<name> --
    # and a glob("*") over it returns exactly those three directory names and
    # finds nothing. That cost one stub run; the depth-bounded search above
    # already knew the layout, which is the whole reason it exists.
    _rg_hits = find_all_markers("coatnet_resgated_ep10_top3_inference.py")
    if not _rg_hits:
        raise RuntimeError(
            "no coatnet_resgated_ep10_top3_inference.py found under "
            f"{RESGATED_DIR}; is the resgated dataset mounted?")
    _rg_root = _rg_hits[0]
    _sys.path.insert(0, str(_rg_root))
    _resg = importlib.import_module("coatnet_resgated_ep10_top3_inference")

    # Free our GPUs first: their pipeline runs two isolated T4 shards and asks
    # for both devices. Ours are done with by here.
    gc.collect()
    if str(device).startswith("cuda"):
        torch.cuda.empty_cache()

    _rg_csv = Path("/kaggle/working/_resgated_arm.csv")
    _rg_receipt = _resg.run_submission(
        competition_root=Path(root),   # `root` is find_test_root()'s string
        artifact_root=_resg.find_artifact_root(),
        output_path=_rg_csv,
    )
    print(f"[resgated] {_rg_receipt.get('status', '?')}", flush=True)

    _rg_df = pd.read_csv(_rg_csv).set_index("StudyInstanceUID")
    _missing = [i for i in ids if i not in _rg_df.index]
    if _missing:
        # Joined on study_id, never on position. A silent positional join is the
        # exact fault class that would look like a weak arm rather than a bug.
        raise RuntimeError(
            f"the resgated arm returned no row for {len(_missing)} studies, "
            f"first {_missing[:3]}")
    _rg_p = _rg_df.loc[list(ids), list(LAB)].to_numpy(dtype=np.float64)
    if not np.isfinite(_rg_p).all():
        raise RuntimeError("the resgated arm produced non-finite probabilities")

    # ONE VOTE PER INDEPENDENT PIPELINE, which is a rule and not a knob: the
