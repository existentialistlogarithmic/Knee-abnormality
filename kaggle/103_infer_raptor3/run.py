...
def main():
    ...
    _rg = sorted(Path(RESGATED_DIR).glob("*"))
    _rg_root = next((d for d in [Path(RESGATED_DIR)] + _rg
                     if (d / "coatnet_resgated_ep10_top3_inference.py").exists()), None)
    if _rg_root is None:
        raise RuntimeError(
            f"RESGATED_DIR {RESGATED_DIR} holds no "
            f"coatnet_resgated_ep10_top3_inference.py; found {[str(d) for d in _rg][:8]}")
