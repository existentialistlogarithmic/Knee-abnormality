...
def main():
    ...
    # FOREIGN PIPELINES (E121, E123). Whole inference packages from OTHER
    # authors, each mounted as a CC0 dataset and driven through its own
    # `run_submission`. We do not reimplement their windowing: each declares its
    # own geometry contract -- six slot budgets, variable windows per study --
    # and rebuilding that from the contract would be a second chance to get it
    # subtly wrong for no benefit.
    #
    # WHY THEY EARN VOTES WHEN E117's SIX SAME-AUTHOR CHECKPOINTS DID NOT. Our
    # four CoAtNet arms correlate 0.905-0.986 with EACH OTHER. These sit at
    # 0.872 and 0.879 with us. A union pays for DISAGREEMENT, not rank, and
    # E117's six were inside our band and lost.
    #
    # THE COLLISION THIS ISOLATION EXISTS FOR: both packages ship a file called
    # `cnx_dicom_geometry.py`. With both directories on sys.path the first
    # import wins and the second package would run on the FIRST one's geometry
    # -- silently, producing plausible numbers from the wrong preprocessing. So
    ...
    for _fa in FOREIGN_ARMS:
        ...
        if _miss:
            # Joined on study_id, never on position. A positional join is the
            # fault class that reads as a weak arm rather than as a bug.
            raise RuntimeError(
                f"foreign arm {_fa['name']} returned no row for {len(_miss)} "
                f"studies, first {_miss[:3]}")
        _pr = _df.loc[list(ids), list(LAB)].to_numpy(dtype=np.float64)
        if not np.isfinite(_pr).all():
            raise RuntimeError(f"foreign arm {_fa['name']} produced non-finite values")
        _foreign.append((_fa["name"], rankpct(_pr)))

    # ONE VOTE PER INDEPENDENT PIPELINE, and the arithmetic has to earn that.
    # By here `ranks` ALREADY blends the pipelines above it, so rank-meaning it
    # against the newcomers would give each newcomer a full share and split one
    # share between the incumbents. The first version of this block did exactly
    # that and its log printed "1/2" -- true of the code, false of the intent.
    # So the prior blend is weighted by HOW MANY pipelines it carries.
    ...
    # NO `rankpct` ON THE PRIOR. E127 measured what that one call costs.
    #
    # `ranks` here is already a MEAN of `_n_prior` rank columns, so its
    # variance is reduced by exactly the averaging that makes two pipelines
    # count as two. Re-ranking it puts that variance back to full uniform, and
    # then weighting it by `_n_prior` over-counts it: on independent uniform
    # inputs, measured at n = 20,000, the newcomer lands at 1/sqrt(5) = 0.446
    # influence where one vote of three is 1/sqrt(3) = 0.578.
    #
    # E124 saw the right defect -- a newcomer rank-meaned 50/50 against an
    # already-blended composite takes half the ensemble, 1/sqrt(2) = 0.707 --
    # and overshot the correction. Dropping the call lands it exactly.
    #
    # It matters here more than the size suggests: E128 measured d4 at 0.9302
    # and global96 at 0.9305 on gold-58, each ABOVE our own four-arm CoAtNet
    # composite at 0.9223. The arm being discounted is the strongest one.
