...
# All ensemble members must share this, or averaging compares different inputs.
...
def main() -> int:
    ...
    for path in checkpoints:
        ...
        # Every member must have been fed the same way, or averaging is invalid.
        recorded = state.get("slice_subsample")
        if recorded not in (None, SLICE_SUBSAMPLE_EXPECTED):
            raise SystemExit(f"{path.name} trained with slice_subsample={recorded}, "
                             f"expected {SLICE_SUBSAMPLE_EXPECTED} — geometries differ, "
                             "averaging them would be invalid")
        model.eval()
        models.append(model)
    ...
    print(f"ensembling {len(models)} folds")
    model.eval()
