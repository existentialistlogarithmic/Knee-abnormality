...
def to_rank(values: np.ndarray) -> np.ndarray:
    """Each labeler's non-abstaining values to [0,1] positions, per finding.

    The two ladders are not commensurable as they stand — one is a stipulated
    probability, the other a rung index — so they are ranked before they are
    averaged. NaN means the labeler abstained on that slot.
    """
    ...
    for i in range(values.shape[1]):
        ...
        if count == 1:
            out[seen, i] = 0.5
            continue
        # Average ranks within ties. argsort().argsort() would break ties by
        # array position, inventing an order between equal scores — and the
        # lexicon emits only five distinct values, so ties are the common case
        # rather than a corner. That arbitrary order is pure noise, and it is
        # noise that would be baked into 4,407 studies' training targets.
        column = column[seen].astype(float)
        order = np.argsort(column, kind="mergesort")
        ranks = np.empty(count, float)
        ...
        out[seen, i] = ranks / (count - 1)
    return out


def fuse(lexicon: np.ndarray, machine: np.ndarray) -> np.ndarray:
    """Union two labelers by coverage. NO FREE PARAMETERS, deliberately.

    Exactly one speaks -> take it. Both speak -> average their ranks. Neither
    -> abstain. A rule that picked the better labeler per finding would be
    fitting twelve choices to 58 studies and would report a number that means
    nothing.
    """
...
def main() -> int:
    ...
    # The question is not "does this beat the lexicon", it is "does adding this
    # to the lexicon beat the lexicon". The first run of this kernel asked the
    # first question, answered no at 0.7526 against 0.769, and stopped — while
    # the union of the two scored +0.049 above either, with a paired interval
    # excluding zero. That was very nearly a good idea discarded for asking the
    # wrong thing.
    lexicon_dir = find_marker("soft_labels.parquet")
    combined_macro = float("nan")
    if lexicon_dir is not None:
        soft = pd.read_parquet(lexicon_dir / "soft_labels.parquet").set_index(
            "StudyInstanceUID")
        ids = gold.StudyInstanceUID.astype(str)
        shared = [s for s in ids if s in soft.index]
        if len(shared) == len(ids):
            lexicon_scores = soft.loc[ids, FINDINGS].to_numpy(dtype=float)
            channels = soft.loc[ids, [f"{f}__channel" for f in FINDINGS]].to_numpy()
            lexicon_scores[channels == "absent"] = np.nan     # its abstain channel
            machine_scores = np.where(
                np.array([[s[name] == "not_mentioned" for name in FINDINGS]
                          for s in gold_states]), np.nan, ranks)
            union = fuse(lexicon_scores, machine_scores)
            # An abstaining slot still needs a position: silence sits below
            # anything either labeler asserted.
            lex_macro, _ = macro_auc(expert, np.nan_to_num(lexicon_scores, nan=-1.0))
            combined_macro, combined_per = macro_auc(
                expert, np.nan_to_num(union, nan=-1.0))
            print(f"\nlexicon alone on the same footing: {lex_macro:.4f}")
            print(f"union of the two:                  {combined_macro:.4f}")
            print(f"union abstain rate:                {np.isnan(union).mean():.1%}")
        else:
            print(f"\nlexicon labels cover {len(shared)}/{len(ids)} gold studies; "
                  "skipping the union check")
    else:
        print("\nno soft_labels.parquet mounted; cannot check the union")
    ...
    if abstain > 0.5:
        ...
        return 1
    # Gate on the union, falling back to the standalone number only when the
    # lexicon labels are not mounted to compare against.
    if combined_macro == combined_macro:
        if combined_macro <= lex_macro:
            print(f"The union ({combined_macro:.4f}) does not beat the lexicon "
                  f"alone ({lex_macro:.4f}). This labeler adds nothing. Stopping "
                  "before the other 4,349.")
            return 0
        print(f"The union adds {combined_macro - lex_macro:+.4f} over the lexicon "
              f"alone, so these states are worth having even though the model "
              f"scores {macro:.4f} by itself.")
    elif macro < 0.769:
        print("No lexicon labels to compare against, and worse than 0.769 alone. "
              "Stopping before the other 4,349.")
        return 0
